...
public class MedicalRecordServiceImpl implements MedicalRecordServiceInterface {
  ...
  public MedicalRecord findCompleteById(Long clinicId) {
    ...
    MedicalRecord recordWithVaccines =
        medicalRecordQueryRepository
            .findWithVaccines(clinicId)
            .orElseThrow(
                () -> new EntityNotFoundException("Medical record not found with ID: " + clinicId));

    MedicalRecord recordWithTreatments =
        medicalRecordQueryRepository
            .findWithTreatments(clinicId)
            .orElseThrow(
                () -> new EntityNotFoundException("Medical record not found with ID: " + clinicId));
  ...
  private void saveTreatments(MedicalRecord medicalRecord) {
    if (medicalRecord.getTreatments() != null) {
      List<Treatment> treatments =
          medicalRecord.getTreatments().stream()
  ...
  private void updateAnamnesis(MedicalRecord existing, Anamnesis updatedAnamnesis) {
    if (updatedAnamnesis != null) {
      ...
      BeanUtils.copyProperties(
          updatedAnamnesis, existingAnamnesis, "id", "version", "vaccines", "medicalRecord");
  ...
  private void updateVaccines(Anamnesis existingAnamnesis, List<Vaccine> newVaccines) {
    Map<Long, Vaccine> existingVaccines =
        existingAnamnesis.getVaccines().stream()
            .collect(Collectors.toMap(Vaccine::getId, Function.identity()));

    List<Vaccine> updatedVaccines =
        newVaccines.stream()
            .map(
                newVaccine -> {
                  if (newVaccine.getId() == null) {
                    ...
                    Vaccine existingVaccine =
                        Optional.ofNullable(existingVaccines.get(newVaccine.getId()))
                            .orElseThrow(
                                () ->
                                    new EntityNotFoundException(
                                        "Vaccine not found with ID: " + newVaccine.getId()));
                    BeanUtils.copyProperties(
                        newVaccine, existingVaccine, "id", "version", "anamnesis");
    ...
    newTreatments.forEach(
        newTreatment -> {
          if (newTreatment.getId() == null) {
            ...
            BeanUtils.copyProperties(
                newTreatment, existingTreatment, "id", "version", "medicalRecord");
          }
        });
  }
    ...
    newDiagnostics.forEach(
        newDiagnostic -> {
          if (newDiagnostic.getId() == null) {
            ...
            BeanUtils.copyProperties(
                newDiagnostic, existingDiagnostic, "id", "version", "medicalRecord");
          }
        });
  }
