...
public class AppointmentServiceImpl implements AppointmentServiceInterface {
    ...
    public Appointment findById(Long appointmentId) {
        ...
        return appointmentRepository.findById(appointmentId).orElseThrow(() -> new EntityNotFoundException("Appointment not found with ID: " + appointmentId));
    }
    ...
    public String findVeterinarianName(Appointment appointment) {
        ...
        return Optional.ofNullable(appointment.getMedicalService()).map(MedicalService::getClinic).map(VeterinaryClinic::getVeterinarian).map(User::getFirstName).orElse("Veterinarian not assigned");
    }

    @Override
    @Transactional
    public Appointment createClientAppointment(String dateStr, Long petId, Long serviceId, Long clinicId, User client) {
        appointmentValidator.validateClientAppointmentCreation(dateStr, petId, serviceId, clinicId, client);
    ...
    @Override
    @Transactional
    public Appointment createReceptionistAppointment(Appointment appointment, Long serviceId, Long receptionistId) {
        appointmentValidator.validateReceptionistAppointmentCreation(appointment, serviceId, receptionistId);
    ...
    public Appointment reschedule(Long appointmentId, Appointment updatedAppointment) {
        ...
        MedicalService service = medicalServiceService.findById(updatedAppointment.getMedicalService().getId());
    ...
    private Appointment buildClientAppointment(LocalDate date, Pet pet, MedicalService service, VeterinaryClinic clinic, User customer) {
    ...
    private void updateAppointmentSchedule(Appointment existing, Appointment updated, MedicalService service) {
